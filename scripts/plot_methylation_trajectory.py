...
def plot_tissue(df, pcs, tissue, outdir, window=10, alpha=0.05, frac_sig=0.05, n_boot=1000):
    ...
    for sex, ax in [("male", axes[0]), ("female", axes[1])]:
        ...
        ax.fill_between(ages, lo, hi, color="#f3a6b6", alpha=0.3)
        ax.plot(ages, means, color="#1f77b4", lw=2)
        for a, m, s in zip(ages, means, sigs):
            ax.plot(a, m, "*" if s else "o", color="red" if s else "#1f77b4")
        ax.set_title(sex)
        ax.set_xlabel("Age")
...
def main(pc_table, outdir="./plots_methyl", window=10, alpha=0.05, frac_sig=0.05, n_boot=1000):
...
if __name__ == "__main__":
    ...
    p.add_argument("--outdir", default="./plots_methyl")
